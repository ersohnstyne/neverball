...
extern "C" void score_steam_init_hs(int hs_index, int incl_set)
{
#if NB_STEAM_API==1
    if (!online_init)
        online_init = 1;
    ...
    if (incl_set != 0)
    {
        sprintf(coin_rank_name, "Most Coins - %s", set_name_n(hs_index));
        sprintf(time_rank_name, "Best Time - %s", set_name_n(hs_index));
    }
    else
    {
...
extern "C" int score_steam_hs_loading(void)
{
#if NB_STEAM_API==1
...
extern "C" int score_steam_hs_online(void)
{
#if NB_STEAM_API==1
    if (!online_init) return 0;
    return
...
extern "C" void score_steam_hs_save(int coins, int time)
{
#if NB_STEAM_API==1
