...
static int help_action(int tok, int val)
{
    ...
    case HELP_DEMO:
        ...
        if (demo_replay_init(current_platform == PLATFORM_PC ? demos[val] : demos_xbox[val],
...
static int page_introduction(int id)
{
    ...
    if (help_page_current == 1)
        gui_multi(id, _("The goal of Switchball is to get your ball to the\\end of the level and board the gyrocopter.\\Once you have completed a level\\the next will be unlocked and so on.\\Eventually, new worlds will be unlocked as well."), GUI_SML, gui_wht, gui_wht);
    if (help_page_current == 2)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("The Checkpoint resurrects\\your ball. If anything bad\\happens, you will start over\\from here."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
    ...
    if (help_page_current == 3)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("The Timer shows how fast you have\\completed the level. By completing\\a level as fast as you can, you can\\earn bronze, silver and gold medals,\\which will reward Achievements\\if you collect enough of them."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
...
static void controls_pc(int id)
{
    ...
    sprintf_s(s_rotate_new, dstSize, "Use %c / %c buttons to rotate the view.\\Hold %s for faster view rotation.", config_get_d(CONFIG_KEY_CAMERA_R), config_get_d(CONFIG_KEY_CAMERA_L), SDL_GetKeyName(config_get_d(CONFIG_KEY_ROTATE_FAST)));
#else
    sprintf(s_rotate_new, "Use %c / %c buttons to rotate the view.\\Hold %s for faster view rotation.", config_get_d(CONFIG_KEY_CAMERA_R), config_get_d(CONFIG_KEY_CAMERA_L), SDL_GetKeyName(config_get_d(CONFIG_KEY_ROTATE_FAST)));
#endif
    ...
    gui_multi(id, _(s_rotate_new), GUI_SML, gui_wht, gui_wht);
    gui_space(id);
...
static int page_modes(int id)
{
    ...
    if ((jd = gui_vstack(id)))
    {
        ...
        if ((server_policy_get_d(SERVER_POLICY_PLAYMODES_UNLOCKED_MODE_CAREER) || campaign_career_unlocked()) && server_policy_get_d(SERVER_POLICY_PLAYMODES_ENABLED_MODE_CAREER))
        {
    ...
    if ((jd = gui_vstack(id)))
    {
        ...
        if (account_get_d(ACCOUNT_SET_UNLOCKS) > 0 && (server_policy_get_d(SERVER_POLICY_PLAYMODES_UNLOCKED_MODE_CAREER) || campaign_career_unlocked()
#if NB_STEAM_API==0 && NB_EOS_SDK==0
            || config_cheat()
#endif
            ))
        {
            if ((accessibility_get_d(ACCESSIBILITY_SLOWDOWN) < 100 || config_cheat()) || !server_policy_get_d(SERVER_POLICY_PLAYMODES_ENABLED_MODE_CHALLENGE))
                gui_label(jd, _("Challenge Mode"), GUI_SML, gui_gry, gui_red);
            else
                gui_label(jd, _("Challenge Mode"), GUI_SML, 0, 0);

            if (!server_policy_get_d(SERVER_POLICY_PLAYMODES_ENABLED_MODE_CHALLENGE) && server_policy_get_d(SERVER_POLICY_EDITION) == 0)
...
static int page_modes_special(int id)
{
    ...
    if ((jd = gui_vstack(id)))
    {
        ...
        {
#if NB_STEAM_API==0 && NB_EOS_SDK==0
            //if ((accessibility_get_d(ACCESSIBILITY_SLOWDOWN) < 100 || config_cheat() || (config_get_d(CONFIG_SMOOTH_FIX) && video_perf() < 55)))
#else
            //if ((accessibility_get_d(ACCESSIBILITY_SLOWDOWN) < 100 || (config_get_d(CONFIG_SMOOTH_FIX) && video_perf() < 55)))
#endif
            ...
            else if ((server_policy_get_d(SERVER_POLICY_PLAYMODES_UNLOCKED_MODE_HARDCORE) || campaign_hardcore_unlocked()))
            {
                ...
                gui_multi(jd,
                    _("Same as Challenge Mode. You must play the entire game\\"
                        ...
                        "and the game cannot be saved."), GUI_SML, gui_wht, gui_wht);
            }
            else
            {
                gui_label(jd, _("Hardcore Mode"), GUI_SML, gui_gry, gui_red);
...
static int page_morphs_and_generators(int id)
{
    ...
    if (help_page_current == 1)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("Roll into a morph to switch\\to the ball presented by the\\hologram. There are four different\\balls, Marbleball, Metalball,\\Airball and Powerball."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
    ...
    if (help_page_current == 2)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("When you have the Powerball,\\roll into a generator to charge\\the ball with a powerup.\\There are three different powerups,\\Jump, Boost and Magnetic."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
...
static int page_machines(int id)
{
    ...
    if (help_page_current == 1)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("When you have the Airball,\\roll onto the pump to inflate\\the Airball with helium.\\The Airball will then fly\\for a brief period of time."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
    ...
    if (help_page_current == 2)
    {
        if ((jd = gui_hstack(id)))
        {
            if ((kd = gui_vstack(jd)))
            {
                gui_multi(kd, _("There are many different machines\\in Switchball. Some machines\\have a switch connected to them.\\Roll onto the button to turn\\on and off."), GUI_SML, gui_wht, gui_wht);
                gui_filler(kd);
            }
            gui_space(jd);
            if ((kd = gui_vstack(jd)))
        ...
        if (help_open)
        {
            ...
            if ((jd = gui_harray(id)))
            {
                ...
                else
                {
                    ...
                    gui_start(jd, _("More"), GUI_SML, HELP_NEXT, 0);
                }
            }
        }
        else
