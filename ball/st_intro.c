    ...
    if (d)
    {
        if (config_tst_d(CONFIG_JOYSTICK_BUTTON_A, b) || config_tst_d(CONFIG_JOYSTICK_BUTTON_START, b))
        {
            ...
            if (val == -1)
            {
                ...
                return networking_connected() == 1 ? goto_state(MAINTENANCE_HOLD ? &st_server_maintenance : (networking_error() > -3 ? &st_title : &st_intro_accn_disabled)) : goto_state(networking_connected() == -1 ? &st_intro_waitinternet : &st_intro_nointernet);
#else
                return goto_end_support(networking_connected() == 1 ? (MAINTENANCE_HOLD ? &st_server_maintenance : (networking_error() > -3 ? &st_title : &st_intro_accn_disabled)) : (networking_connected() == -1 ? &st_intro_waitinternet : &st_intro_nointernet));
#endif
            }
            else
            {
...
static void intro_timer(int id, float dt)
{
    ...
    if (iframe > 160 && !intro_done)
    {
        ...
        {
#if defined(SKIP_END_SUPPORT)
            networking_connected() == 1 ? goto_state(MAINTENANCE_HOLD ? &st_server_maintenance : (networking_error() > -3 ? &st_title : &st_intro_accn_disabled)) : goto_state(networking_connected() == -1 ? &st_intro_waitinternet : &st_intro_nointernet);
#else
            goto_end_support(networking_connected() == 1 ? (MAINTENANCE_HOLD ? &st_server_maintenance : (networking_error() > -3 ? &st_title : &st_intro_accn_disabled)) : (networking_connected() == -1 ? &st_intro_waitinternet : &st_intro_nointernet));
#endif
            return;
        }
        else
    ...
    sprintf_s(introattr, dstSize, "gui/intro/%04d.jpg", iframe);
#else
    sprintf(introattr, "gui/intro/%04d.jpg", iframe);
#endif
...
static int intro_restore_action(int tok, int val)
{
    ...
    case RESTORE_ACCEPT:
        ...
        case RESTORE_FULLSCREEN:
            goto_state(&st_null);
            r = video_mode(config_get_d(CONFIG_GRAPHIC_RESTORE_VAL1), w, h);
            if (r)
            {
                remove_all();
#if defined(SKIP_END_SUPPORT)
        ...
        case RESTORE_RESOLUTION:
            goto_state(&st_null);
            r = video_mode(f, config_get_d(CONFIG_GRAPHIC_RESTORE_VAL1), config_get_d(CONFIG_GRAPHIC_RESTORE_VAL2));
            if (r)
            {
                remove_all();
#if defined(SKIP_END_SUPPORT)
        ...
        case RESTORE_DISPLAY:
            goto_state(&st_null);
            int oldDisp = config_get_d(CONFIG_DISPLAY);
            ...
            if (r)
            {
                ...
                config_set_d(CONFIG_DISPLAY, oldDisp);
            break;
        ...
        case RESTORE_TEXTURES:
            ...
            if (r)
            {
                ...
                config_set_d(CONFIG_TEXTURES, oldText);
            break;
        }
...
static int intro_restore_gui(void)
{
    ...
    if ((id = gui_vstack(0)))
    {
...
#if _WIN32 && !defined(__EMSCRIPTEN__) && !_CRT_SECURE_NO_WARNINGS
            _itoa_s(restore_val_1, gfx_target_values_v2, 256, 2);
#elif _WIN32
            _itoa(restore_val_1, gfx_target_values_v2, 2);
#else
            itoa(restore_val_1, gfx_target_values_v2, 2);
#endif
            gfx_target_values = (restore_val_1 == 0 ? gfx_target_values_v2 : _("Off"));
            break;
        ...
        if (doubles) {
#if _WIN32 && !defined(__EMSCRIPTEN__) && !_CRT_SECURE_NO_WARNINGS
            sprintf_s(restore_doubles, dstSize, _("The game, that you've set up some graphics\\has a crash. Would you restore them now?\\%s: %i x %i"), gfx_target_name, restore_val_1, restore_val_2);
#else
            sprintf(restore_doubles, _("The game, that you've set up some graphics\\has a crash. Would you restore them now?\\%s: %i x %i"), gfx_target_name, restore_val_1, restore_val_2);
#endif
        ...
        {
#if _WIN32 && !defined(__EMSCRIPTEN__) && !_CRT_SECURE_NO_WARNINGS
            sprintf_s(restore_singles, dstSize, _("The game, that you've set up some graphics\\has a crash. Would you restore them now?\\%s: %s"), gfx_target_name, gfx_target_values);
#else
            sprintf(restore_singles, _("The game, that you've set up some graphics\\has a crash. Would you restore them now?\\%s: %s"), gfx_target_name, gfx_target_values);
#endif
...
{
    audio_play(tok == GUI_BACK ? AUD_BACK : AUD_MENU, 1.0f);
    switch (tok)
    {
    case GUI_BACK:
        goto_state(&st_null); /* bye! */
...
static int server_maintenance_enter(struct state *st, struct state *prev)
{
    ...
    if ((id = gui_vstack(0)))
    {
        ...
        if (networking_standalone())
            ...
            gui_multi(id, _("It might take a wihle until\\the server maintenance is finished."),
                GUI_SML, gui_wht, gui_wht);
