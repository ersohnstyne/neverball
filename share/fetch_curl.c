...
int fetch_enable(int enable)
{
    ...
    if (fetch_enabled != old_value)
    {
        if (fetch_enabled) {
            if (!fetch_init()) {
                log_errorf("Failure to enable fetch!\n");
                fetch_enabled = 0;
                return 0;
            }}
        else fetch_quit();
...
/*
 * HACK: Prevent Soft- and Hardlocks to get in there! - Ersohn Styne
 */
#define CURLM_FETCHINFO_ABORT(_fi) do { \
    if (_fi->dest_file) { \
        fs_close(_fi->dest_file); \
        _fi->dest_file = NULL; \
    } if (_fi->callback.done) { \
        struct fetch_event *fe = create_fetch_event(); \
        if (fe) { \
            fe->callback = _fi->callback.done; \
            fe->callback_data = _fi->callback.data; \
            fe->extra_data = create_extra_done(0); \
            fetch_dispatch_event(fe); \
        } \
    } \
    unlink_and_free_fetch_info(_fi); \
    } while (0)
